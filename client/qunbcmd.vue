...
const schema = Schema.dict(
  Schema.object({
    disabledCommands: Schema.array(Schema.string())
      .description('禁用的指令列表')
      .default([]),
  })
).description('群禁用指令配置');
...
const fetchConfigs = async () => {
  try {
    const data = await send('get-qun-bcmd');
...
const submitUpdates = async () => {
  try {
    await send('update-qun-bcmd', configDict.value);
